...
- (void)didTouch {
    if ([self isEndDateIsSmallerThanCurrent:self.date] && ![self checkItDateisToday:self.date]) {
...
-(BOOL)checkItDateisToday:(NSDate*)date{
    ...
    NSString * dateObject = [dateFormat stringFromDate:date];
    
    NSString *todayDate = [dateFormat stringFromDate:[NSDate date]];
    
    
    
    if ([dateObject isEqualToString:todayDate]) {
